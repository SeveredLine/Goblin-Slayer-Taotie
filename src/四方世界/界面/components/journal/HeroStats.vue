<template>
  <div ref="container" class="flex flex-col gap-6 text-[#d0c0a0]">
    ...
    <div class="flex h-[320px] gap-2">
      
      <!-- 1. 雷达图区域 -->
      <div class="hero-stats-panel relative flex w-1/2 flex-col items-center justify-center rounded border border-[#4a3e35] bg-[#00000033]">
        ...
        <svg viewBox="0 0 200 200" class="h-full w-full p-4 drop-shadow-[0_0_10px_rgba(197,157,95,0.3)]">
          ...
          <!-- 动态数据多边形 -->
          <polygon ref="radarPoly" :points="radarPoints" fill="rgba(197, 157, 95, 0.2)" stroke="#c59d5f" stroke-width="2" class="origin-center transform transition-none"/>
      ...
      <!-- 2. 纸娃娃装备栏 -->
      <div class="hero-stats-panel relative flex w-1/2 flex-col items-center rounded border border-[#4a3e35] bg-[#2a221e] bg-[url('https://www.transparenttextures.com/patterns/leather.png')]">
        <div class="hero-name mt-2 mb-4 w-3/4 border-b border-[#c59d5f]/30 text-center font-serif text-lg text-[#c59d5f]">{{ hero.姓名 || 'Unknown' }}</div>
        ...
        <div class="relative z-10 grid w-full grid-cols-2 gap-x-12 gap-y-2 px-4">
          <EquipSlot class="equip-slot" icon="fa-helmet-safety" :item="hero.装备?.防具?.头部" /> <!-- 头 -->
          <EquipSlot class="equip-slot" icon="fa-gem" :item="hero.装备?.饰品?.项链" /> <!-- 颈 -->
          
          <EquipSlot class="equip-slot" icon="fa-shirt" :item="hero.装备?.防具?.身体" /> <!-- 胸 -->
          <EquipSlot class="equip-slot" icon="fa-mitten" :item="hero.装备?.防具?.手部" /> <!-- 手 -->
          
          <EquipSlot class="equip-slot" icon="fa-khanda" :item="hero.装备?.武器?.主手" /> <!-- 主手 -->
          <EquipSlot class="equip-slot" icon="fa-shield-halved" :item="hero.装备?.武器?.副手" /> <!-- 副手 -->
          
          <EquipSlot class="equip-slot" icon="fa-socks" :item="hero.装备?.防具?.腿部" /> <!-- 腿 -->
          <EquipSlot class="equip-slot" icon="fa-boot" :item="hero.装备?.防具?.脚部" /> <!-- 脚 -->
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-between gap-4 px-2">
      <!-- HP Vial -->
      <div class="vial-panel group relative flex flex-1 items-center gap-3 overflow-hidden rounded-lg border border-[#4a3e35] bg-[#0c0a0a] p-3">
...
class="absolute bottom-0 w-full bg-linear-to-t from-red-900 to-red-600"
        ...
        <!-- 底部发光装饰 -->
        <div class="absolute bottom-0 left-0 h-1/2 w-full bg-red-900/10 blur-xl transition-colors group-hover:bg-red-900/20"></div>
      </div>

      <!-- MP Vial -->
      <div class="vial-panel group relative flex flex-1 flex-row-reverse items-center gap-3 overflow-hidden rounded-lg border border-[#4a3e35] bg-[#0c0a0a] p-3">
        <div class="relative h-24 w-4 overflow-hidden rounded-full border border-[#555] bg-[#222]">
          <div
ref="mpBar"
class="absolute bottom-0 w-full bg-linear-to-t from-blue-900 to-blue-500"
        ...
        <div class="absolute right-0 bottom-0 h-1/2 w-full bg-blue-900/10 blur-xl transition-colors group-hover:bg-blue-900/20"></div>
      </div>
    </div>
    ...
    <div class="buffs-panel border-t border-[#4a3e35] pt-4">
      ...
      <div class="custom-scrollbar flex gap-2 overflow-x-auto pb-2">
        <div v-for="n in 5" :key="n" class="buff-icon flex h-8 w-8 cursor-help items-center justify-center rounded border border-[#444] bg-[#222] hover:border-[#c59d5f]">
