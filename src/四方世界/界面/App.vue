<template>
  <div class="fixed inset-0 z-999999 overflow-hidden bg-black font-serif select-none">
    ...
    <div v-else-if="gameState === 'GAME'" class="flex h-full w-full flex-col bg-[#110e0e] text-[#a89f91]">
      <!-- 顶部：状态栏 (Top Bar) -->
      <header
        class="relative z-20 flex h-16 items-center justify-between border-b-2 border-[#4a3e35] bg-linear-to-b from-[#2c2520] to-[#1a1614] px-6 shadow-lg"
      >
        <!-- 左侧：装饰性标题 -->
        ...
        <!-- 中间：时钟与世界状态 (SVG 模拟时钟) -->
        <div
          class="absolute top-0 left-1/2 flex h-24 w-64 -translate-x-1/2 flex-col items-center justify-center rounded-b-xl border-x-2 border-b-2 border-[#c59d5f] bg-[#1a1614] shadow-xl"
        >
      ...
      <div class="relative flex flex-1 overflow-hidden">
        <!-- 左侧：剧场 (Chat) -->
        <main
          class="relative flex flex-1 flex-col bg-[#e6dccf] bg-[url('https://www.transparenttextures.com/patterns/cream-paper.png')]"
        >
        ...
        <!-- 右侧：手札 (Journal) -->
        <aside class="relative z-20 flex w-112.5 flex-col border-l-4 border-[#2c2520] bg-[#1a1614] shadow-2xl">
          <JournalView />
        </aside>
      </div>
    </div>
