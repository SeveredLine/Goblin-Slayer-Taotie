<template>
  <div class="fixed inset-0 z-99999 flex items-center justify-center overflow-hidden bg-[#030303] font-serif text-white select-none">
    <!-- 背景层：动态模糊与宇宙尘埃 -->
    <div class="pointer-events-none absolute inset-0 opacity-40 mix-blend-screen">
      <div class="absolute inset-0 animate-pulse bg-[radial-gradient(ellipse_at_center,rgba(60,40,90,0.4)_0%,transparent_60%)] opacity-60 duration-10000"></div>
      <div class="absolute inset-0 animate-[spin_120s_linear_infinite] bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-30"></div>
      <div class="absolute inset-0 animate-[spin_180s_linear_infinite_reverse] bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-15"></div>
    </div>
    
    <!-- 暗角遮罩 (Vignette) 聚焦中心视觉 -->
    <div class="pointer-events-none absolute inset-0 z-10 bg-[radial-gradient(circle_at_center,transparent_20%,#000_100%)]"></div>

    <!-- 内容容器：增加镜头级控制层 -->
    <div class="relative z-20 flex h-full w-full items-center justify-center overflow-hidden perspective-[1500px]">
      <div ref="stage" class="relative flex h-full w-full max-w-7xl transform-gpu items-center justify-center px-10 text-center">
      <!-- 渲染所有名单，通过 GSAP 控制显隐 -->
      <div v-for="(scene, sIndex) in creditsSequence" :key="'scene-'+sIndex" class="scene-container pointer-events-none absolute inset-0 flex flex-col items-center justify-center opacity-0">
        
        <!-- 动态标题 (职能) - 电影级长间距 -->
        <div class="role-text mb-8 transform-gpu font-serif text-xs font-bold tracking-[0.8em] text-[#a89f91] uppercase drop-shadow-2xl md:mb-14 md:text-xl">
          {{ scene.role }}
        </div>

        <!-- 动态名单列表 -->
        <div class="names-container flex flex-wrap items-center justify-center gap-x-12 gap-y-6 md:gap-x-24 md:gap-y-12">
          <div
  ...
  <!-- 跳过提示 - 高级磨砂玻璃质感按钮 -->
    ...
    >
      ...
      <div class="absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/10 to-transparent transition-transform duration-700 group-hover:translate-x-full"></div>
    </button>

    <!-- 电影级进度条 - 细长发光线条 -->
    <div class="absolute bottom-0 left-0 z-30 h-[2px] w-full bg-white/5">
      <div ref="progressBar" class="relative h-full w-0 bg-linear-to-r from-transparent via-[#c59d5f] to-[#ffdeb3] shadow-[0_0_10px_#c59d5f]">
        <!-- 进度条头部发光点 -->
         <div class="absolute top-1/2 right-0 h-[4px] w-[15px] -translate-y-1/2 bg-white blur-[2px]"></div>
      </div>
    </div>
  </div>
</template>
...
const playSequence = async () => {
  ...
  scenes.forEach((sceneEl, index) => {
    ...
    if (styleType === 'hero') {
      ...
      sceneTl.fromTo(
        nameChars,
        { opacity: 0, scale: 4, z: 800, filter: 'blur(30px)', rotationX: () => gsap.utils.random(-80, 80), rotationY: () => gsap.utils.random(-80, 80) },
        { opacity: 1, scale: 1, z: 0, filter: 'blur(0px)', rotationX: 0, rotationY: 0, duration: 1.8, stagger: { amount: 0.8, from: 'random' }, ease: 'power4.out' },
        0.4
      );
      ...
      sceneTl.fromTo(
        ...
        { opacity: 1, y: 0, rotationX: 0, filter: 'contrast(1) brightness(1)', duration: 1.2, stagger: 0.15, ease: 'back.out(2)' },
        0.4
      );
      // 每个字符在被翻出时瞬间闪亮
      sceneTl.fromTo(nameChars,
      ...
      sceneTl.fromTo(
        ...
        { opacity: 1, y: 0, filter: 'blur(0px) contrast(1)', scale: 1, duration: 2.5, stagger: { amount: 1.5, from: 'edges' }, ease: 'sine.out' },
        0.3
      );
      ...
      sceneTl.fromTo(
        ...
        { opacity: 1, scale: 1, filter: 'blur(0px) brightness(1)', y: 0, duration: 3.5, stagger: 0.6, ease: 'power2.out' },
        0.4
      );
      // 让字母之间产生呼吸感
      sceneTl.fromTo(nameChars, 
      ...
      sceneTl.fromTo(
        ...
        { opacity: 1, filter: 'blur(0px)', rotationZ: 0, scale: 1, duration: 1.8, stagger: { amount: 1.2, from: 'random' }, ease: 'power2.out' },
        0.4
      );
    }
